...
function drawChart() {
  ...
  models.flatSeating.data.forEach((dataPoint, index) =>
    rows[index].push(dataPoint)
  );
  models.setSeating.data.forEach((dataPoint, index) =>
    rows[index].push(dataPoint)
  );
  models.customSeating.data.forEach((dataPoint, index) =>
    rows[index].push(dataPoint)
  );
...
function createServiceFlow(model) {
  ...
  for (let i = 0; i < model.data.length; i++) {
    ...
    let seatsFilled = currentSeatingIntervals.reduce(
...
function refreshTables() {
  ...
  flatSeatingGen(
    ...
    venueSettings.duration,
    venueSettings.seatingIntervals
  );
  setSeatingGen(
    venueSettings.capacity,
    venueSettings.duration,
